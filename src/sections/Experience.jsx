...
const experiences = [
  {
    ...
    company: "FAST NUCES",
    duration: "2026",
    description: "TA for Digital Logic Design & Probability & Statistics. Guided students through complex circuit concepts and statistical analysis via tutorials and assignment support.",
  },
  {
    role: "Co-Head, API Competition Module",
    company: "Dev Day — FAST NUCES",
    duration: "2026",
    description: "Designed and supervised technical API competitions. Coordinated tasks, managed evaluation processes, and collaborated with student teams to optimize participant experience.",
  },
  {
    role: "Freelance Web Developer",
    company: "Personal & Client Projects",
    duration: "2023 – Present",
    description: "Built responsive MERN stack web applications for clients. Delivered full-stack workflows including REST APIs, database integration, and deployment.",
  },
];
...
const Experience = () => {
  ...
  const thresholds = React.useMemo(() => Array.from({ length: numExperiences }, (_, i) => (i + 1) / numExperiences), [numExperiences]);
            ...
            <div className="relative w-full max-w-7xl hidden md:block">
              ...
              <div className="relative flex justify-between mt-0">
                {experiences.map((exp, idx) => {
                  ...
                  return <ExperienceItem key={idx} exp={exp} idx={idx} start={start} end={end} scrollYProgress={scrollYProgress} layout="desktop" />;
                })}
              </div>
            </div>
            {/* Mobile */}
            <div className="relative w-full max-w-md md:hidden">
              ...
              <div className="relative flex flex-col gap-10 ml-10 mt-6 pb-28">
                {experiences.map((exp, idx) => {
                  const start = idx === 0 ? 0 : thresholds[idx - 1];
                  const end = thresholds[idx];
                  return <ExperienceItem key={idx} exp={exp} idx={idx} start={start} end={end} scrollYProgress={scrollYProgress} layout="mobile" />;
                })}
              </div>
            </div>
          </div>
